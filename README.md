...
- **Card pack management (v0.2.0+)** — detects card art mods (`.pck` overriding `card_art/MegaCrit.Sts2.Core.Models.Cards.*` or shipping `card_portraits/`), shows them in a separate panel on Character Select with checkboxes (enable/disable), ↑/↓ arrows (reorder), and an explicit `1./2./3.` order number per row. Multiple packs can be active simultaneously; **the pack at the top of the list wins** for overlapping cards. The panel is collapsible (▼/▶ header) and scrollable for long lists. Toggle/reorder writes to STS2's `settings.save` and triggers the same restart modal as character-skin changes.
