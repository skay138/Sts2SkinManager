...
- **자동 인식** — `<sts2>/mods/` 하위를 **재귀적으로** 스캔해 세 종류 모드를 자동 감지 (예: `mods/캐릭터/`, `mods/아트워크/`, `mods/유틸/` 처럼 카테고리 폴더로 정리해도 인식. `.pck` 파일명이 다르면 어떤 깊이든 OK — 같은 이름의 `.pck` 가 둘 이상 있으면 첫 번째만 유지하고 경고 로그). STS2 본체도 `mods/` 를 재귀적으로 스캔하므로 DLL+manifest+pck 가 묶인 모드도 깊이 상관없이 작동해요 — **단, 옮긴 뒤 원래 `mods/<modName>/` 폴더는 반드시 삭제**해주세요. 안 그러면 두 위치 모두 발견돼서 `DUPLICATE_ID` 에러가 납니다:
